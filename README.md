...
Cada canal tem seu próprio `stable.json`, assinado com Ed25519. Cada release é imutável, tem sequência
...
2. Criar ZIP imutável em `releases/<canal>/` e calcular SHA-256.
3. Incrementar `releaseSequence`, atualizar o payload `signed` e assinar sua
   serialização canônica.
4. Revisar o commit e publicar no branch `main`.

Nunca edite uma release publicada ou reutilize `releaseSequence` dentro do mesmo canal.
